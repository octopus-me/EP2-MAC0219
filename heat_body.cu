...
__global__ void jacobi_kernel(double *d_h, double *d_g, int n) {
    ...
    if (i > 0 && i < n - 1 && j > 0 && j < n - 1) {
        if (!(i >= BODY_START_X && i <= BODY_END_X && j >= BODY_START_Y && j <= BODY_END_Y)) {
            ...
            d_g[i * n + j] = d_h[i * n + j]; // Manter temperatura fixa do corpo quente
        }
...
// Função para inicializar a matriz com condições de contorno e corpo quente
...
int main(int argc, char *argv[]) {
    ...
    // Calcular speedup
    double speedup = elapsed_time_cpu / (elapsed_time_gpu / 1000.0);
